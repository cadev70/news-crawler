...
export class TwitterCrawler extends BaseCrawler<PlatformConfig> {
    ...
    /**
     * Fetch tweets from all configured accounts or a specific target
     * @param target Optional target account to crawl (e.g., "wojespn")
     */
    protected async fetchArticles(target?: string): Promise<Article[]> {
        ...
        if (target) {
            ...
            if (!matchedAccount) {
                log.warn(`Target account "${target}" not found in configured Twitter accounts`);
                return articles;
            }
            accountsToCrawl = [matchedAccount];
            log.info(`Targeting specific Twitter account: @${matchedAccount}`);
        }
